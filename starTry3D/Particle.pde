...
class Particle {
  ...
  void update (float turn) {
     ...
     */
    PVector current = new PVector(radius * cos(angle)+width/2, tilt + 20 * cos(angle + 3.5)-height/8, radius * sin(angle));
...
void isoLine(PVector begin, PVector end, float angle) {
   ...
   */
  stroke(200+200*cos(angle), 127+127*cos(angle));
  strokeWeight(2);
